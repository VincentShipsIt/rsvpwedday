...
const wedding = new Date(2027, 5, 12, 15, 0);

describe("resolveWeddingDate", () => {
	it("uses the date the couple set", () => {
		expect(resolveWeddingDate(wedding, [{ startsAt: new Date(2020, 0, 1) }])).toEqual({
			date: wedding,
			source: "set",
		});
	});
	...
	it("falls back to the earliest event by date, not by list order", () => {
		const events = [
			{ startsAt: new Date(2027, 5, 13, 11, 0) },
			{ startsAt: new Date(2027, 5, 11, 19, 0) },
			{ startsAt: new Date(2027, 5, 12, 15, 0) },
		];
		expect(resolveWeddingDate(null, events)).toEqual({
			date: new Date(2027, 5, 11, 19, 0),
			source: "derived",
		});
	});
...
describe("dayOffset", () => {
	it("is zero anywhere on the wedding day itself", () => {
		expect(dayOffset(new Date(2027, 5, 12, 0, 1), wedding)).toBe(0);
		expect(dayOffset(new Date(2027, 5, 12, 23, 59), wedding)).toBe(0);
	});

	it("counts calendar days, so late the night before is one day before", () => {
		expect(dayOffset(new Date(2027, 5, 11, 23, 0), wedding)).toBe(-1);
		expect(dayOffset(new Date(2027, 5, 13, 1, 0), wedding)).toBe(1);
	});

	it("counts across a month boundary", () => {
		expect(dayOffset(new Date(2027, 4, 30, 12, 0), wedding)).toBe(-13);
	});
	...
	it("is unaffected by a daylight-saving shift between the two days", () => {
		...
		const marchWedding = new Date(2027, 2, 29, 12, 0);
		expect(dayOffset(new Date(2027, 2, 27, 12, 0), marchWedding)).toBe(-2);
	});
});
