...
describe("buildIllustrationPrompt", () => {
	...
	it("falls back to the placement brief when the block has no copy yet", () => {
		const prompt = buildIllustrationPrompt({ placement: "ITEM", title: "  " }, context);
	...
	it("forbids lettering, faces and a painted-paper border in every prompt", () => {
		for (const placement of ["HERO", "GALLERY", "CARDS"] as const) {
...
describe("illustrationAspectRatio", () => {
	it("frames each placement the way the site crops it", () => {
		...
		expect(illustrationAspectRatio("ITEM")).toBe("3:2");
	});
});
