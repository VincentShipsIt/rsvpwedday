...
/*
 * One-time move of the fixed home page and the guide into `Page`/`Block` rows. Guarded by a durable
 * version marker, adopting databases that already have pages without changes, and it runs on every deploy
...
export async function migrateToPages(): Promise<"migrated" | "skipped"> {
	return db.$transaction(
		async (tx) => {
			...
			if (await tx.bootstrapState.findUnique({ where: { id: "pages-v1" } })) return "skipped";
			await tx.bootstrapState.create({ data: { id: "pages-v1" } });
			if ((await tx.page.count()) > 0) return "skipped";

			const [siteContent, guideSections, faqEntries] = await Promise.all([
				tx.siteContent.findUnique({ where: { id: 1 }, include: { translations: true } }),
				tx.guideSection.findMany({
				...
				tx.faqEntry.findMany({ orderBy: { sortOrder: "asc" }, include: { translations: true } }),
			]);
